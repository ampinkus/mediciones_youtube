...
export const actualizarStream = async (req, res) => {
  try {
    ...
    // ---------------- 2) Validación días ----------------
    const diasSeleccionados = Array.isArray(dias_medicion)
      ? dias_medicion
      : dias_medicion?.trim()
      ? [dias_medicion]
      : [];
    ...
    let diasMedicionFinal = diasSeleccionados.join(",");
    ...
    cfgUpdates.dias_medicion = diasMedicionFinal;

    // ---------------- 6) Guardar sólo si hay cambios ----------------
    const tareas = [];
